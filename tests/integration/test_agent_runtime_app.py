...
def agent_app(monkeypatch: pytest.MonkeyPatch) -> AgentEngineApp:
    ...
    agent_runtime.logger = MagicMock()
...
async def test_agent_stream_query(agent_app: AgentEngineApp) -> None:
    ...
    Tests that the agent returns valid streaming responses for an expense report.
    """
    # Pass a valid expense payload that auto-approves
    payload = {
        "data": {
            "amount": 45.00,
            "submitter": "Dave",
    ...
    # Check for valid output in the response
    has_valid_output = False
    for event in events:
        validated_event = Event.model_validate(event)
        output = validated_event.output
        if output is not None and "status" in output:
            assert output["status"] == "APPROVED"
            assert output["expense"]["amount"] == 45.00
            has_valid_output = True
            break

    assert has_valid_output, "Expected at least one event with valid output dict"
